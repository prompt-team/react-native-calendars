...
const BOUNCINESS = 6;
const MIN_HEIGHT = 56;
const CLOSED_HEIGHT = 126; // header + 1 week
const WEEK_HEIGHT = 46;
const KNOB_CONTAINER_HEIGHT = 32;
  ...
  handlePanResponderMove = (e, gestureState) => {
    ...
    if (!this.props.horizontal) {
      ...
      if (this.state.position === POSITIONS.CLOSED) {
        if (this._wrapperStyles.style.height < (this.closedHeight - WEEK_HEIGHT + KNOB_CONTAINER_HEIGHT) && gestureState.dy < 0) {
        ...
        this._weekCalendarStyles.style.opacity = this._wrapperStyles.style.height < (this.closedHeight - WEEK_HEIGHT + KNOB_CONTAINER_HEIGHT) ? 0 : this._wrapperStyles.style.height <= this.closedHeight ? 1 : Math.min(1, Math.max(1 - gestureState.dy / 100, 0));
      }
    }
    ...
    return (
      <Animated.View
        ...
        style={{
          ...
          opacity: position === POSITIONS.CLOSED ? 1 : 0
        }}
