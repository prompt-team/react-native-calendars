  ...
  handlePanResponderMove = (e, gestureState) => {
    ...
    if (!this.props.horizontal) {
      ...
      if (this.state.position === POSITIONS.CLOSED) {
        if (this._wrapperStyles.style.height < (this.closedHeight - WEEK_HEIGHT) && gestureState.dy < 0) {
        ...
        this._weekCalendarStyles.style.opacity = this._wrapperStyles.style.height < (this.closedHeight - WEEK_HEIGHT) ? 0 : this._wrapperStyles.style.height <= this.closedHeight ? 1 : Math.min(1, Math.max(1 - gestureState.dy / 100, 0));
      }
    }
